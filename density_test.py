...
path = './density/'  #待检测图片的位置

# 创建创建一个存储检测结果的dir
result_path = './static/density_results'
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(path):        
        ...
        #r_image.show() 显示检测结果
        print(num_people)
        image_save_path = './static/density_results/' + portion[1]       
        ...
        # 使用cursor()方法获取操作游标 
        cursor = db.cursor()
        #cursor.execute('create table density(location varchar(40) primary key,num_people varchar(40))')
        if num_people <= 3:
            density_people = '稀疏'
        elif num_people <= 8:
            density_people = '适中'
        ...
        sql = "INSERT INTO density(location, num_people) VALUES ( ' "+loc+" ', ' "+density_people+" ' )"
        # 执行sql语句
        cursor.execute(sql)
        # 提交到数据库执行
        db.commit()
